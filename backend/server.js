...
// --- RANKING DE GRUPOS (Promedio de puntos) ---
app.get("/ranking-grupos", async (req, res) => {
  try {
    const result = await db.query(`
            SELECT g.name as group_name, ROUND(AVG(puntajes.puntos), 1) as puntos
            FROM grupos g
            ...
            JOIN (
                ...
                LEFT JOIN matches m ON p.match_id = m.id
                GROUP BY u.id
            ) as puntajes ON mg.user_id = puntajes.id
            GROUP BY g.name
            ORDER BY puntos DESC
            LIMIT 10
        `);
    ...
    res.status(500).json({ error: "Error en ranking grupos" });
  }
});
